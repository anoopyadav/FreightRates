...
def fill_in_the_blanks(results, begin, end):
    missing_days = set(get_date_range(begin, end)) - \
                   set([row['day'].isoformat() for row in results])

    for day in missing_days:
        results.append({'day': datetime.strptime(
            day, '%Y-%m-%d').date(), 'average_price': 'null'})
...
def get_date_range(begin, end):
    ...
    return [(end - timedelta(days=x)).isoformat()
            for x in range(0, end.day - begin.day + 1)]
